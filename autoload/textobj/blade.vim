let s:start_pattern = '\v\@(section|if|unless|foreach|forelse|for|while|push|can|cannot|hasSection|php|verbatim)'
...
function! textobj#blade#select_a()
  ...
  if searchpair(s:start_pattern,'',s:end_pattern, s:flags, s:skip_pattern)
    let end_pos = getpos('.')
